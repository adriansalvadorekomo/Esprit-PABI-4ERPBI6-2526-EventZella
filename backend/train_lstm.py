...
mlflow.set_experiment("lstm_final_price_forecast")
...
# We'll use the last 'look_back' points to predict the next point, then iterate
horizon = 6
...
for i in range(horizon):
    ...
    X_pred = last_sequence.reshape((1, 1, look_back))
    # Predict next point
    next_point = model.predict(X_pred, verbose=0)
    predictions.append(next_point[0, 0])
    # Update sequence: remove first element, add prediction
    last_sequence = np.append(last_sequence[1:], [[next_point[0, 0]]], axis=0)
...
with mlflow.start_run():
    ...
    plt.legend()
    plt.grid(True)
    plot_path = f"models/lstm_forecast_plot_{version}.png"
    ...
    # Also save as pickle for consistency (though .h5 is preferred for TensorFlow)
    with open(f"models/lstm_model_{version}.pkl", "wb") as f:
        pickle.dump(model, f)
    with open("lstm_model.pkl", "wb") as f:
        pickle.dump(model, f)
    # Save scaler
    with open(f"models/lstm_scaler_{version}.pkl", "wb") as f:
        pickle.dump(scaler, f)
    with open("lstm_scaler.pkl", "wb") as f:
        pickle.dump(scaler, f)

print(f"\n? LSTM training done - version: {version}")